...
import { registerCustomProjections } from '../runtime/projections'
...
export const SystemConfig = (props: SystemConfigProps & CoordinateConfig): React.ReactElement => {
  ...
  const [proj4NameInput, setProj4NameInput] = useState((!wkid && crs?.name) ? crs.name : '')
  const [proj4NameInvalid, setProj4NameInvalid] = useState(false)
  // translate
  ...
  const proj4CrsNameLabel = translate('proj4CrsName')
  const proj4CrsNameHint = translate('proj4CrsNameHint')
  const invalidProj4NameTips = translate('invalidProj4NameTips')
  // units translate
  ...
  useEffect(() => {
    ...
    setProj4NameInput((!wkid && crs?.name) ? crs.name : '')
    setProj4NameInvalid(false)
    if (apiLoaded) {
  ...
  const wkidAccept = (value) => {
    // Empty/cleared WKID → proj4 custom CRS mode; never treat as invalid
  ...
  const handleProj4NameAccept = (value: string) => {
    value = value?.trim()
    if (!value) return
    if (proj4.defs(value)) {
      setProj4NameInvalid(false)
      const proj4Crs = { name: value }
      setItemLabel(value)
      setSystemName(value)
      if (!id) {
        // new system
        onWkidChangeSave('', proj4Crs)
      } else {
  ...
  const getUnitOptions = (): React.JSX.Element[] => {
    ...
    // proj4 custom CRS (no WKID): offer all units so the user can pick
    if (isNaN(curWkidNum)) return allUnits
      ...
      <div className='w-100 h-100 system-config-panel'>
        {allDataLoaded
          ? <Fragment>
            ...
            {!curWkid &&
              <SettingSection title={proj4CrsNameLabel}>
                <SettingRow>
                  <TextInput
                    size='sm'
                    type='text'
                    className='w-100'
                    value={proj4NameInput}
                    title={proj4CrsNameHint}
                    placeholder='e.g. YAM94'
                    onChange={e => { setProj4NameInput(e.target.value); setProj4NameInvalid(false) }}
                    onAcceptValue={handleProj4NameAccept}
                    aria-label={proj4CrsNameLabel}
                  />
                </SettingRow>
                {proj4NameInvalid &&
                  <div className='text-break system-name d-flex align-items-center'>
                    <WarningOutlined color={theme.sys.color.error.main} />
                    <div className='invalid-tips'>{invalidProj4NameTips}</div>
                  </div>
                }
              </SettingSection>
            }
            ...
            <SettingSection>
              ...
              >
                <Select
                  ...
                  disabled={(!curWkid && !proj4NameInput) || wkidInvalid || proj4NameInvalid}
                >
                  {getUnitOptions()}
                </Select>
              </SettingRow>
