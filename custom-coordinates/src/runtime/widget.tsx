/** @jsx jsx */
import {
  React, jsx, type AllWidgetProps, utils, moduleLoader, lodash, ReactResizeDetector, hooks, type IMState, ReactRedux, css, getAppStore,
...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (currentMapView?.view) {
        currentMapView.view.cursor = 'default'
      }
  ...
  const canShowInClient = async (selectedSystem: CoordinateConfig): Promise<boolean> => {
    ...
    // proj4 custom CRS (no WKID): handled entirely on the client via proj4
    if (!wkid && crs?.name && proj4.defs(crs.name)) return true
  ...
  const displayOnClient = async (mapPoint, threeDPoint?) => {
    ...
    // ── proj4 custom CRS (no WKID) ─────────────────────────────────────────
    // If a proj4 definition exists for this CRS name, project on the client
    // without touching any Esri SpatialReference / geometry-service code.
    if (!wkid && crs?.name && proj4.defs(crs.name)) {
      // Resolve geographic lon/lat from whatever SR the map is in
      const mapSrForProj4 = new SpatialReference({ wkid: mapWkid.current })
      let lon: number
      let lat: number
      if (mapSrForProj4.isWebMercator) {
        // Web Mercator → WGS84 lon/lat (no server call needed)
        const lngLat = webMercatorUtils.xyToLngLat(mapPoint.x, mapPoint.y)
        lon = lngLat[0]
        lat = lngLat[1]
      } else {
        // Assume geographic (WGS84 or similar datum)
        lon = mapPoint.longitude ?? mapPoint.x
        lat = mapPoint.latitude ?? mapPoint.y
      }
      // Project WGS84 → custom CRS using proj4
      const [projX, projY] = proj4('WGS84', crs.name, [lon, lat])
      const nlsUnit = unitToNls(displayUnit) || ''
      const suffix = nlsUnit ? ` ${nlsUnit}` : ''
      const orderXy = displayOrder === DisplayOrderType.xy
      orderXy
        ? setGeoInfo(`${toFormat(projX)} ${toFormat(projY)}${suffix}`)
        : setGeoInfo(`${toFormat(projY)} ${toFormat(projX)}${suffix}`)
      // 3-D elevation / eye-altitude (unchanged from normal path)
      const view3d = currentJimuMapView?.view
      if (view3d?.type === '3d') {
        _setEyeInfo()
        if (threeDPoint) _setElevInfo(threeDPoint)
      }
