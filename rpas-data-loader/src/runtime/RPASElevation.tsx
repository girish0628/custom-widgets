...
import { Config, getProjectionOptions } from '../config';
...
export default class RPASElevation extends React.PureComponent<Props, State> {
  constructor(props) {
    super(props);
    const now = new Date();
  ...
  handleUpload = async () => {
    ...
    const validation = ValidationService.validateRPASInputs(
    ...
    const taskUrl = config?.rpasGPUrl;
    if (!taskUrl) {
    ...
    try {
      ...
      if (result.jobStatus === 'job-succeeded') {
        this.showAlert('RPAS data uploaded and processed successfully!', 'success');
